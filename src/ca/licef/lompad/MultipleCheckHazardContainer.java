 ...
 * @author I7 MAX
 */
public class MultipleCheckHazardContainer extends TextForm{
    ...
    public MultipleCheckHazardContainer(String title, boolean isLine, boolean isMultipleContainer, boolean isOneLine) {
        super(title, isLine, isMultipleContainer, isOneLine);
        mediator = new LangstringMediator(this, values);
    }
    
    void addFormContent() {
        MultipleCheckHazardComponent date = new MultipleCheckHazardComponent();
        date.setMinimumSize(new Dimension(10, 63));
        date.setPreferredSize(new Dimension(10, 63));
        date.setMaximumSize(new Dimension(1000, 63));
        addComponent(date);
    }
